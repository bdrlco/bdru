...
{
    // Convert bdrunit:// to bdrunit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bdrunit://"))
    {
        uri.replace(0, 12, "bdrunit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BDRUnit.lnk";
}
...
{
    return GetAutostartDir() / "bdrunit.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BDRUnit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bdrunit-qt [" + tr("command-line options") + "]                     " + "\n";
