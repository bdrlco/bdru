...
#if defined(NDEBUG)
# error "BDRUnit cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BDRUnit Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 18000000 * COIN;   //18M Premine 
    }
        else
    {
        nSubsidy = 0 * COIN;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (TestNet())
    {
        ...
        nCoinbaseMaturity = 60; // test maturity is 10 blocks
    }
